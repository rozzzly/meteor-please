...
exports.initJson = function(cwd) {
  // Build JSON
  console.log("Configuring paty.json settings file...".cyan);

  var PATH_TO_PATY_SETTINGS_JSON = path.resolve(cwd, 'paty.json');
  var patyJson = {};
  ...
  prompt.get({properties: {
    ...
    if (err) {
      ...
      if (res.sshpass) {
        patyJson.servers[0].password = res.sshpass;
      } else {
        patyJson.servers[0].pem = res.sshkey;
      }
      jsonFile.writeFile(PATH_TO_PATY_SETTINGS_JSON, patyJson, function(err) {
        if (err) return console.error(err.red);
        console.log("You can now use paty setup && deploy!".blue);
      });
    }
  });
};
