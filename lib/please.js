...
function please(current_dir, config) {
	...
	var verbosity = this.config.verbosity || true;
...
please.prototype.init = function() {
	...
	var PATH_TO_PATY_SETTINGS_JSON = path.resolve(this.current_dir, 'paty.json');

	prompt.message = "[paty.json]".magenta;
	prompt.start();
	if(fs.existsSync(PATH_TO_PATY_SETTINGS_JSON)) {
		// Already initialised
		prompt.get({
			properties: {
				continue: {
					description: "You've already initialised paty in this project. Start over?",
					default: 'n',
					pattern: /^[YNyn\s]{1}$/
				}
			}
...
please.prototype.setup = function () {
	...
	var config_appRemoteTargetPath = '/var/www/meteor-please/' + config_appName + '/bundle/programs/web.browser/app/';
...
please.prototype.deploy = function() {
	...
	var config_appRemoteTargetPath = '/var/www/meteor-please/' + config_appName;
	...
	bash.on('close', function(code) {
		if(code) {
			...
			// BUILD SUCCESS!!!
			// now, scp bundle to server

			// mkdir
			taskList.execute('Configuring installation directory...', {
				/*
				 * add meteor-please to this path so deployed apps can be made accessible via group perms
				 *
				 * this way root (ssh) access is not required, just chown the meteor-please folder to the same group
				 * as the configured ssh user (I used the 'developers' group)
				 *
				 * @author rozzzly <pclienau@gmail.com>
				 * @date 7/5/15 3:34 am CST
				 */
				command: 'sudo mkdir -p ' + config_appRemoteTargetPath

			});
			// chown
			taskList.execute('setting ownership of installation directory...', {
				command: 'sudo chown ' + config_appUser + ':developers ' + config_appRemoteTargetPath
			});
			// chmod
			taskList.execute('setting permissions on installation directory...', {
				command: 'sudo chmod 775 ' + config_appRemoteTargetPath
			});
			// scp file
			...
			taskList.copy('Configuring systemd daemon...', {
				 ...
				 * @date 7/5/15 3:34 am CST
				 */
				dest: '/etc/systemd/system/meteor-please/' + config_appName + '.service',
				vars: {
					...
					// ^ is redundant, that's defined already with the rest of the configs above
...
please.prototype.reconfig = function() {
	...
	taskList.copy('Reconfiguring systemd daemon...', {
		src: PATH_TO_SYSTEMD_CONF,
		dest: '/etc/systemd/system/meteor-please/' + this.config.appName + '.service',
		vars: {
			appRootUrl: config_appRootUrl,
			appPort: config_appPort,
...
please.prototype.delete = function() {
	...
	taskList.execute('Removing app files...', {
		command: 'sudo rm -rf /var/www/meteor-please/' + this.config.appName
	});
	taskList.execute('Removing daemon...', {
		command: 'sudo rm -rf /etc/systemd/system/meteor-please/' + this.config.appName + '.service'
	});
	// send restart to nginx daemon
	taskList.execute('Restarting nginx... ', {
		command: 'sudo systemctl restart nginx'
